...
public class XWRefreshFooter: XWRefreshComponent {
    ...
    /** 如果列表数据不超过totalCount，将footview隐藏*/
    public var totalCount = 0;
    //MARK: 私有的
    
    var delegate:TableDataCountDelegate?;
        ...
        if let _ = newSuperview {
            ...
            tmpClass.reloadDataClosure = { (totalCount:Int) -> Void in
                ...
                if self.automaticallyHidden == true {
                    ...
                    self.hidden = totalCount <= self.totalCount;
                    
                    //print("foot hidden:\(self.hidden)");
                    
                    self.delegate?.doSendDataCount(totalCount);
                }
            }
...
protocol TableDataCountDelegate{
