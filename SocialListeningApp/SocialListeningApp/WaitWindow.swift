...
extension UIViewController {
    ...
    func pleaseWaitWithDefaultImages(){
        let timeInterval: Int = 100;
        var imagesArray = Array<UIImage>();
        let loadingImage = UIImage(named: "loading");
        for i in 1...7 {
            let loadImg = UIImage(CGImage: (loadingImage?.CGImage)!, scale: (loadingImage?.scale)!, orientation: UIImageOrientation(rawValue: 10*i)!);
            imagesArray.append(loadImg)
        }
        SwiftNotice.wait(imagesArray, timeInterval: timeInterval)
...
class SwiftNotice: NSObject {
    ...
    static func wait(imageNames: Array<UIImage> = Array<UIImage>(), timeInterval: Int = 0) {
        ...
        let x:CGFloat = CGFloat((frame.width - width) / 2)
        
        if imageNames.count > 0 {
            
            let frame2 = CGRectMake(x, x, width, width)

            if imageNames.count > timerTimes {
                
                let iv = UIImageView(frame: frame2)
            ...
            ai.frame = CGRectMake(x, x, width, width)
            ai.startAnimating()
...
//        window.hidden = false
    ...
    static func getRealCenter() -> CGPoint {
        if UIApplication.sharedApplication().statusBarOrientation.hashValue >= 3 {
            ...
            return CGPoint(x: AppWidth/2, y: AppHeight/2);
        }
    }
}
